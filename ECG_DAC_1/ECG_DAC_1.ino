...
void loop() {
  for (int i = 0; i < numSamples; i++) {
    ...
    uint16_t valueC = (uint16_t)((adjustedData[2][i] + 5.0) / 10.0 * 4095);
    uint16_t valueD = (uint16_t)((adjustedData[3][i] + 5.0) / 10.0 * 4095);
    ...
    delay(1); // Adjust this delay as needed for real-time playback
  }
}